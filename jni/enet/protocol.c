...
static ENetPeer *
enet_protocol_handle_connectU(ENetHost* host, ENetProtocolHeader2* header, ENetProtocol* command)
{
...
{
    size_t useNew = (host->usingNewPacket == 2);
    ENetProtocolHeader* header = NULL;
    ENetProtocolHeader2* newHeader = NULL;
    ...
    if (host->receivedDataLength < (size_t) & ((ENetProtocolHeader*)0)->sentTime && !useNew)
        return 0;
    if (host->receivedDataLength < (size_t) & ((ENetProtocolHeader2*)0)->sentTime && useNew)
        return 0;

    if (useNew) newHeader = (ENetProtocolHeader2*)host->receivedData;
    else header = (ENetProtocolHeader*)host->receivedData;

    if (useNew) peerID = ENET_NET_TO_HOST_16(newHeader->peerID);
    else peerID = ENET_NET_TO_HOST_16(header->peerID);
    sessionID = (peerID & ENET_PROTOCOL_HEADER_SESSION_MASK) >> ENET_PROTOCOL_HEADER_SESSION_SHIFT;
    flags = peerID & ENET_PROTOCOL_HEADER_FLAG_MASK;
    peerID &= ~(ENET_PROTOCOL_HEADER_FLAG_MASK | ENET_PROTOCOL_HEADER_SESSION_MASK);

    if (useNew) headerSize = (flags & ENET_PROTOCOL_HEADER_FLAG_SENT_TIME ? sizeof(ENetProtocolHeader2) : (size_t) & ((ENetProtocolHeader2*)0)->sentTime);
    else headerSize = (flags & ENET_PROTOCOL_HEADER_FLAG_SENT_TIME ? sizeof(ENetProtocolHeader) : (size_t) & ((ENetProtocolHeader*)0)->sentTime);
    ...
    if (flags & ENET_PROTOCOL_HEADER_FLAG_COMPRESSED)
    {
        ...
        if (useNew) memcpy(host->packetData[1], newHeader, headerSize);
        else memcpy(host->packetData[1], header, headerSize);
    ...
    while (currentData < &host->receivedData[host->receivedDataLength])
    {
        ...
        case ENET_PROTOCOL_COMMAND_CONNECT:
            if (peer != NULL)
                goto commandError;
            if (useNew) peer = enet_protocol_handle_connectU(host, newHeader, command);
            else peer = enet_protocol_handle_connect(host, header, command);
            if (peer == NULL)
                goto commandError;
            break;
            ...
            (command->header.command & ENET_PROTOCOL_COMMAND_FLAG_ACKNOWLEDGE) != 0)
        {
            ...
            if (useNew) sentTime = ENET_NET_TO_HOST_16(newHeader->sentTime);
            else sentTime = ENET_NET_TO_HOST_16(header->sentTime);
...
{
    if (host->usingNewPacket == 0 || host->usingNewPacket == 2) {
    ...
    else if (host->usingNewPacket == 1) 
    {
    enet_uint8 headerData[sizeof(ENetProtocolHeader2) + sizeof(enet_uint32)];
    ENetProtocolHeader2* header = (ENetProtocolHeader2*)headerData;
    ENetPeer* currentPeer;
    int sentLength;
    size_t shouldCompress = 0;
    ...
    int v7 = (host->peers->address.port);
    enet_uint16 v8 = rand() % (v7 + 1);
    enet_uint16 v9 = rand();
    header->a = htons(v8);
    header->b = htons(v8 ^ v7);
    header->c = ntohs(v9 & 0xF7DF | 0x9005);
    ...
    while (host->continueSending)
            ...
            ++currentPeer)
        {
            ...
            host->commandCount = 0;
            host->bufferCount = 1;
            host->packetSize = sizeof(ENetProtocolHeader2);
            ...
            if (host->headerFlags & ENET_PROTOCOL_HEADER_FLAG_SENT_TIME)
            {
                ...
                host->buffers->dataLength = sizeof(ENetProtocolHeader2);
            }
            else
                host->buffers->dataLength = (size_t) & ((ENetProtocolHeader2*)0)->sentTime;

            shouldCompress = 0;
            if (host->compressor.context != NULL && host->compressor.compress != NULL)
            {
                size_t originalSize = host->packetSize - sizeof(ENetProtocolHeader2),
                    compressedSize = host->compressor.compress(host->compressor.context,
                        &host->buffers[1], host->bufferCount - 1,
                        originalSize,
                        host->packetData[1],
