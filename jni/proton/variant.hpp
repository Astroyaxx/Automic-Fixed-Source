...
template <typename T>
inline const std::string to_string(T value)
{
...
class variant_t {
   public:
    ...
    std::string print() {
        switch (get_type()) {
            case vartype_t::TYPE_FLOAT: return to_string(get_float());
            case vartype_t::TYPE_STRING: return get_string();
            case vartype_t::TYPE_VECTOR2: return "x: " + to_string(get_vector2().m_x) + " y: " + to_string(get_vector2().m_y);
            case vartype_t::TYPE_VECTOR3:
                return "x: " + to_string(get_vector3().m_x) + " y: " + to_string(get_vector3().m_y) + " z: " + to_string(get_vector3().m_z);
            case vartype_t::TYPE_UINT32: return to_string(get_uint32());
            case vartype_t::TYPE_INT32: return to_string(get_int32());
            case vartype_t::TYPE_RECT:
                return "x: " + to_string(get_rect().m_x) + " y: " + to_string(get_rect().m_y) + " w: " + to_string(get_rect().m_w) +
                       " h: " + to_string(get_rect().m_h);
            case vartype_t::TYPE_ENTITY:
            case vartype_t::TYPE_COMPONENT:
    ...
    std::string print() {
        ...
        for (int i = 0; i < C_MAX_VARIANT_LIST_PARMS; i++) {
            if (m_variant[i].get_type() == variant_t::vartype_t::TYPE_UNUSED)
                continue;
            ss << "param " << to_string(i) << ": " << m_variant[i].print() + "\n";
        }
        if (ss.str().empty())
            ss.str("(none)");
        return ss.str();
