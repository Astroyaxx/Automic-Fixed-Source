...
int32_t ImGui_ImplAndroid_HandleInputEventGrowtopia(void* handler, CL_Vec2f vec2f, uint8_t type) {
	...
	io.MousePos = ImVec2((int)round(vec2f.x), (int)round(vec2f.y));
    return 0;
}
...
bool ImGui_ImplAndroid_Init(ANativeWindow* window)
{
    ...
    io.BackendPlatformName = "ofxImGui"; // trick reversers into thinking thats its using ofx lmfao
...
void ImGui_ImplAndroid_NewFrame(int screenSizeX, int screenSizeY)
{
    ImGuiIO& io = ImGui::GetIO();
    ...
    int window_width = g_Window ? ANativeWindow_getWidth(g_Window) : screenSizeX;
    int window_height = g_Window ? ANativeWindow_getHeight(g_Window) : screenSizeY;
